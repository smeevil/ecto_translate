defmodule EctoTranslateTest do
  ...
  setup do
    ...
    test_record =
      EctoTranslate.Repo.insert!(%EctoTranslate.ExampleModel{
  ...
  @valid_attributes [
    locale: :nl,
    title: "Een nederlandse titel",
    description: "Een nederlandse beschrijving"
  ]

  test "It should return errors for translated fields if the content is not a string", state do
    result =
      EctoTranslate.set(state[:test_record], locale: :nl, title: 42, description: :this_is_wrong)

    assert {:error,
            [
              {"title", [content: {"is invalid", [type: :string, validation: :cast]}]},
              {"description", [content: {"is invalid", [type: :string, validation: :cast]}]}
            ]} == result
  end

  test "It should return errors for translated fields if the locale is not supported", state do
    result =
      EctoTranslate.set(state[:test_record],
        locale: :gibberish,
        title: "test",
        description: "test"
      )
    ...
    assert {:error,
            [
              {"title",
               [locale: {"The locale 'gibberish' is not supported, supported are" <> _, []}]},
              {"description",
               [locale: {"The locale 'gibberish' is not supported, supported are" <> _, []}]}
            ]} = result
  end
  ...
  test "it returns translated fields", state do
    ...
    assert "Een nederlandse titel" ==
             EctoTranslate.ExampleModel.translated_title(state[:test_record], :nl)
  end

  test "it returns a fallback when for non translated fields", state do
    assert state[:test_record].title ==
             EctoTranslate.ExampleModel.translated_title(state[:test_record], :nl)
  end
  ...
  test "it should translate the state[:test_record] inline using the current gettext locale",
       state do
    EctoTranslate.set(state[:test_record],
      locale: :de,
      title: "Eine deutche titel",
      description: "Ein deutsche umschreibung"
    )
  ...
  test "it should update an existing translation", state do
    :ok =
      EctoTranslate.set(state[:test_record],
        locale: :de,
        title: "Eine neue deutche titel",
        description: "Ein neue deutsche umschreibung"
      )

    1 = EctoTranslate.ExampleModel |> EctoTranslate.Repo.all() |> Enum.count()

    record =
      EctoTranslate.ExampleModel
      |> limit(1)
      |> EctoTranslate.Repo.one()
      |> EctoTranslate.ExampleModel.translate!(:de)
  ...
  test "it can fetch translations for a list of entries" do
    ...
    record =
      EctoTranslate.Repo.insert!(%EctoTranslate.ExampleModel{
        title: "an other entry",
        description: "An other description"
      })

    EctoTranslate.set(record,
      locale: :nl,
      title: "Nog een ingave",
      description: "Nog een andere beschrijving"
    )

    records =
      EctoTranslate.ExampleModel
      |> EctoTranslate.Repo.all()
      |> EctoTranslate.ExampleModel.translate!(:nl)
